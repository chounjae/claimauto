...
const TEMPLATE_OVERVIEW =
  `[헬스장명]과 [계약일]에 헬스장 이용계약을 체결하고 [금액]원을 납부하였습니다. [중단일]에 중도해지를 요청하였으나 업체 측이 환불을 거부/묵살하였습니다. 공정거래위원회 「소비자분쟁해결기준」(체육시설업)에 따라 환불금 [금액]원 지급을 요청합니다.`

const TEMPLATE_DEMAND =
  `공정거래위원회 「소비자분쟁해결기준」(체육시설업)에 따라 계산된 환불금 [금액]원의 즉시 지급을 요구합니다.`
...
const STEPS: Step[] = [
  ...
  },
  {
    ...
    hint: '소비자원 공문이 업체에 발송되면 그 단계에서 합의되는 경우가 많습니다.',
  },
]
...
const FAQS = [
  ...
  },
  {
    ...
    a: 'PT 이용권도 일반 헬스장 이용권과 동일하게 공정거래위원회 「소비자분쟁해결기준」(체육시설업) 적용 대상입니다. 잔여 횟수 기준으로 계산하며, 이미 진행한 횟수를 단가(총액 ÷ 총 횟수)로 계산해 공제한 후 환불받을 수 있습니다. PT와 헬스장 이용권을 묶음 구매한 경우에는 각각 분리해서 계산해야 합니다. 업체가 "진행한 수업료가 비싸다"며 임의로 단가를 높여 적용하는 것은 소비자분쟁해결기준 위반입니다.',
  },
  {
  ...
  },
  {
    ...
    a: '받을 가능성이 높습니다. 다만 정확히 알아두실 것이 있습니다. 소비자분쟁해결기준은 소비자기본법 제16조 제3항에 따라 "분쟁당사자 사이에 별도의 의사표시가 없는 경우 분쟁해결을 위한 합의 또는 권고의 기준"입니다. 그 자체로 계약을 무효로 만드는 강행규정은 아닙니다. 그러나 소비자원 피해구제와 분쟁조정에서 실질적인 판단 기준으로 사용되므로 실무상 힘이 있습니다. 별도로 "환불 불가" 특약은 「약관의 규제에 관한 법률」 제6조·제9조에 따라 무효로 판단될 수 있습니다(개별 사안에 따라 다릅니다). 업체가 계약서 조항을 이유로 거부하면 소비자원에 신고하세요.',
  },
  {
